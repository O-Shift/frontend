...
export interface TransformResult {
  /** Markdown with citation markers rewritten as titled links. */
  content: string;
  /** Every citation found, in first-appearance order. */
  citations: Citation[];
}
...
export function transformCitations(
  ...
  const content = source.replace(CITE_MARKER, (whole, rawUrl: string, rawLabel?: string) => {
...
export function citationIndexFromTitle(title: string | null | undefined): number | null {
  if (!title || !title.startsWith(CITE_TITLE_PREFIX)) return null;
